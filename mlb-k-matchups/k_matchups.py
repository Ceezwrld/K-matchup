...
def format_table(df: pd.DataFrame) -> str:
    show = df.copy()
    for col in ("expected_k_pct", "expected_whiff_pct", "expected_ks", "expected_ks_1x"):
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    p.add_argument(
        ...
        help=(
            f"Assumed batters faced for expected_ks "
            f"(default: {DEFAULT_BATTERS_FACED})"
        ),
    )
    p.add_argument(
        "--lineup-lookback",
...
def main(argv: list[str] | None = None) -> int:
    ...
    log(
        ...
        f"min_usage={args.min_usage}, batters_faced={args.batters_faced}",
    )
