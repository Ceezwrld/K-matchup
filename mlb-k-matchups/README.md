...
12. **Strike% / F-Strike% / Zone%** — season **pitches thrown for strikes** (`Strike%` = Strikes ÷ Pitches), first-pitch strike rate, and zone rate from FanGraphs (Stats API `strikePercentage` fallback). Shown on each pitcher's **Rates / form** row; board header also shows today's **Avg Strike%**. Confirmation / command context only — does not change `expected_ks`.
13. **Total-trust gate (THIN_TOTAL)** — ELITE/STRONG with Exp K ≥ ~5.5 only fully trusts the **juiced total** when STYLE is **WHIFF**. STYLE **GB/FLY** → outlook **THIN_TOTAL** (O3.5 / thin O4.5 only). STYLE **BAL** → soft total-caution note (O4.5 floor). Mix side can still be over; the *number* is not a nuke on BIP-out styles (8/4 Dobbins). Does not change Exp K.
14. **Under confirmation (UNDER_OK)** — SOFT non-SPIKE needs ≥2 of: **GB/FLY style**, **opp contact-heavy BIP**, **Exp K ≤ ~4.2**. Passes → outlook **UNDER_OK**. Fewer confirms → weak under (U6.5+ or pass). SPIKE still vetoes soft U6.
